...
-- Create recipe_ingredients table (table de jointure)
CREATE TABLE IF NOT EXISTS recipe_ingredients (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  recipe_id UUID NOT NULL REFERENCES recipes(id) ON DELETE CASCADE,
  ingredient_id UUID REFERENCES ingredients(id) ON DELETE SET NULL,
  ingredient_name VARCHAR(255) NOT NULL, -- Stocké pour garder le nom même si l'ingrédient est supprimé
...
-- Les utilisateurs peuvent voir les ingrédients de leurs recettes
...
-- View pour faciliter les requêtes (recettes avec nombre d'ingrédients)
